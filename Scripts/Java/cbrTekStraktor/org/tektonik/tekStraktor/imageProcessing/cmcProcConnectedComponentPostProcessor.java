package org.tektonik.tekStraktor.imageProcessing;
...
import org.tektonik.tekStraktor.model.cmcStopWatch;
import org.tektonik.tekStraktor.model.comicPage;
import org.tektonik.tekStraktor.textDatabase.cmcTextDump;
...
import org.tektonik.tools.generalStatPurpose.gpFrequencyDistributionStatFunctions;
import org.tektonik.tools.generalStatPurpose.gpListStatFunctions;
  ...
  //------------------------------------------------------------
	...
	//------------------------------------------------------------
	{
      ...
      for(int i=0;i<cococulst.size();i++)
      {
    	  ...
    	  // Stat
    	  statDAO ster = gatherLETTERStatistics();
	...
	//------------------------------------------------------------
	private statDAO gatherLETTERStatistics()
	//------------------------------------------------------------
	{
		TRACE("gatherLETTERStatistics");
		statDAO x = new statDAO();
		//
		int aantal=0;
		for(int i=0;i<ccb_ar.length;i++)
		{
	...
	//------------------------------------------------------------
	{
		...
		//
		statDAO stc = gatherLETTERStatistics();
		//
		int STARTBUNDEL=100;
...
//System.out.println("BUNDLE [" + tcc_ar[cntr].bundel + "] ELEMs[" + tcc_ar[cntr].counter + "] " + tcc_ar[cntr].MinX + " " + tcc_ar[cntr].MinY + " " + tcc_ar[cntr].MaxX + " " + tcc_ar[cntr].MaxY);
	...
	//------------------------------------------------------------
	{
		...
		statDAO stco = gatherLETTERStatistics();
		int HoogteRange=0;
		int BreedteRange=0;
		switch( proxtol )
		{
