...
import org.tektonik.MachineLearning.cmcMachineLearningModelController;
import org.tektonik.tekStraktor.dao.cmcArchiveDAO;
import org.tektonik.tekStraktor.dao.cmcBookMetaDataDAO;
import org.tektonik.tekStraktor.dao.cmcStatDAO;
import org.tektonik.tekStraktor.dao.cmcTimingDAO;
import org.tektonik.tekStraktor.drawing.cmcGraphController;
import org.tektonik.tekStraktor.featureExtraction.cmcVRExtractMLAndHOGFeatureSet;
import org.tektonik.tekStraktor.featureExtraction.cmcVRMakeTrainingImages;
import org.tektonik.tekStraktor.featureExtraction.cmcVRPublishResults;
import org.tektonik.tekStraktor.featureExtraction.cmcVRRunMachineLearning;
import org.tektonik.tekStraktor.featureExtraction.cmcVRRunTensorFlow;
import org.tektonik.tekStraktor.imageProcessing.cmcProcConnectedComponent;
import org.tektonik.tekStraktor.imageProcessing.cmcProcConnectedComponentPostProcessor;
import org.tektonik.tekStraktor.imageProcessing.cmcProcDetermineThreshold;
import org.tektonik.tekStraktor.imageProcessing.cmcTextBundleReport;
import org.tektonik.tekStraktor.imageProcessing.cmcTimingInfo;
import org.tektonik.tekStraktor.ocr.cmcOCRController;
import org.tektonik.tekStraktor.textDatabase.cmcTextExport;
import org.tektonik.tekStraktor.textDatabase.cmcTextImport;
import org.tektonik.tools.generalImagePurpose.cmcImageRoutines;
import org.tektonik.tools.generalImagePurpose.gpIntArrayFileIO;
import org.tektonik.tools.generalImagePurpose.filters.cmcBulkImageRoutines;
	...
	protected Integer doInBackground() throws Exception
    {
		...
		while (!this.isCancelled()) {
            ...
            case PROCESS_GRADIENT_NARROW : {
            	processGradient( cmcProcEnums.HOG_GRADIENT_DISPLAY_TYPE.NARROW );
            	cSema.setSemaphore(taskTipe); 
            	taskTipe = cmcProcSemaphore.TaskType.DO_NOTHING; 
            	break;
            }
            case PROCESS_GRADIENT_WIDE : {
            	processGradient( cmcProcEnums.HOG_GRADIENT_DISPLAY_TYPE.WIDE );
            	cSema.setSemaphore(taskTipe); 
            	taskTipe = cmcProcSemaphore.TaskType.DO_NOTHING; 
            	break;
            }
            case PROCESS_GRADIENT_MAGNITUDE : {
            	processGradient( cmcProcEnums.HOG_GRADIENT_DISPLAY_TYPE.MAGNITUDE );
            	cSema.setSemaphore(taskTipe); 
            	taskTipe = cmcProcSemaphore.TaskType.DO_NOTHING; 
            	break;
            }
            case PROCESS_GRADIENT_THETA : {
            	processGradient( cmcProcEnums.HOG_GRADIENT_DISPLAY_TYPE.THETA );
            	cSema.setSemaphore(taskTipe); 
            	taskTipe = cmcProcSemaphore.TaskType.DO_NOTHING; 
            	break;
            }
            case PROCESS_VERTICAL_GRADIENT : {
            	processGradient( cmcProcEnums.HOG_GRADIENT_DISPLAY_TYPE.VERTICAL_GRADIENT );
            	cSema.setSemaphore(taskTipe); 
            	taskTipe = cmcProcSemaphore.TaskType.DO_NOTHING; 
            	break;
            }
            case PROCESS_HORIZONTAL_GRADIENT : {
            	processGradient( cmcProcEnums.HOG_GRADIENT_DISPLAY_TYPE.HORIZONTAL_GRADIENT );
            	cSema.setSemaphore(taskTipe); 
            	taskTipe = cmcProcSemaphore.TaskType.DO_NOTHING; 
            	break;
            }
            ...
            case PROCESS_CONVOLUTION_SHARPEN : {
                process_convolution(org.tektonik.tools.generalImagePurpose.filters.cmcProcConvolution.kernelType.SHARPEN);
            	cSema.setSemaphore(taskTipe); 
            	taskTipe = cmcProcSemaphore.TaskType.DO_NOTHING; 
            	break;
            }
            case PROCESS_CONVOLUTION_GLOW : {
                process_convolution(org.tektonik.tools.generalImagePurpose.filters.cmcProcConvolution.kernelType.BLUR);
            	cSema.setSemaphore(taskTipe); 
            	taskTipe = cmcProcSemaphore.TaskType.DO_NOTHING; 
            	break;
            }
            case PROCESS_CONVOLUTION_EDGE : {
                process_convolution(org.tektonik.tools.generalImagePurpose.filters.cmcProcConvolution.kernelType.EDGE);
            	cSema.setSemaphore(taskTipe); 
            	taskTipe = cmcProcSemaphore.TaskType.DO_NOTHING; 
            	break;
            }
            case PROCESS_CONVOLUTION_GAUSSIAN : {
                process_convolution(org.tektonik.tools.generalImagePurpose.filters.cmcProcConvolution.kernelType.GAUSSIAN33);
            	cSema.setSemaphore(taskTipe); 
            	taskTipe = cmcProcSemaphore.TaskType.DO_NOTHING; 
            	break;
            }
            ...
            case PROCESS_TEXTURE : {
            	//process_invert();
            	do_error( "to do controller - textture");
            	cSema.setSemaphore(taskTipe); 
            	taskTipe = cmcProcSemaphore.TaskType.DO_NOTHING; 
            	break;
            }
            ...
            case PREPARE_OCR : {
            	...
            	taskTipe = cmcProcSemaphore.TaskType.DO_NOTHING; 
               break; }
            case VR_SHOW_MODEL : ; // same as SHOW_OCR_FILE
            case SHOW_OCR_FILE : {
            	resetTimerStats();
            ...
            case VR_MAKE_TRAINING_IMAGE_SET: {
            	if( vrMakeTrainingImageSet() ) cSema.setSemaphore(taskTipe); 
            	else cSema.setSemaphore(cmcProcSemaphore.TaskType.SHOUT); // skips the rest
                taskTipe = cmcProcSemaphore.TaskType.DO_NOTHING; 
                break;
                }
            case VR_EXTRACT_FEATURES : {
            	if( doVRExtractFeatures() ) cSema.setSemaphore(taskTipe); 
            	else cSema.setSemaphore(cmcProcSemaphore.TaskType.SHOUT); // skips the rest
                  taskTipe = cmcProcSemaphore.TaskType.DO_NOTHING; 
                 break;
                }
            case VR_TRAIN_MODEL : {
            	if( doVRTrainModel() ) cSema.setSemaphore(taskTipe); 
            	else cSema.setSemaphore(cmcProcSemaphore.TaskType.SHOUT); // skips the rest
                  taskTipe = cmcProcSemaphore.TaskType.DO_NOTHING; 
                 break;
                }
            case VR_EXTRACT_AND_RUN_SINGLE_SET : {
            	if( doVRMakeAndRunSingleSet() ) cSema.setSemaphore(taskTipe);
            	else cSema.setSemaphore(cmcProcSemaphore.TaskType.SHOUT); // skips the rest
            	taskTipe = cmcProcSemaphore.TaskType.DO_NOTHING; 
                break;
            }
...
//irout.writePixelsToFile(work, uncroppedWidth, uncroppedHeigth,"c:\\temp\\cbrtekstraktor\\junkuncrop.png",cmcImageRoutines.ImageType.RGB);
	...
	{
		cPage.cmcImg.workPixels = null;
		cPage.cmcImg.workPixels = ibulk.doMaxLloyd( cPage.cmcImg.pixels , xMSet.getMaxLloydLevel() );
		cPage.cmcImg.moveWorkPixelsToImage();
	}
	...
	{
		cPage.cmcImg.workPixels = null;
		cPage.cmcImg.workPixels = ibulk.doCanny( cPage.cmcImg.pixels , cPage.cmcImg.getWidth() );
		cPage.cmcImg.moveWorkPixelsToImage();
	}
	...
	//-----------------------------------------------------------------------
	private void process_convolution(org.tektonik.tools.generalImagePurpose.filters.cmcProcConvolution.kernelType kernelTipe)
	//-----------------------------------------------------------------------
	{
		cPage.cmcImg.workPixels = null;
	...
	private boolean vrMakeTrainingImageSet()
	{
	   cmcVRMakeTrainingImages ms = null;	
	   switch( xMSet.getMachineLearningType() )
	   {
	   case BAYES      : ;  // same as TENSOR
	   case TENSORFLOW : { ms = new cmcVRMakeTrainingImages( xMSet , logger , cmcProcConstants.VR_TENSOR_IMAGE_WIDTH , cmcProcConstants.VR_TENSOR_IMAGE_HEIGTH , false); break; }
	   case SVM        : { ms = new cmcVRMakeTrainingImages( xMSet , logger , cmcProcConstants.VR_HOG_IMAGE_WIDTH , cmcProcConstants.VR_HOG_IMAGE_HEIGTH , true ); break; }
	   default         : break;
	   }
	...
	//-----------------------------------------------------------------------
	private boolean doVRExtractFeatures()
	//-----------------------------------------------------------------------
	{
		switch( xMSet.getMachineLearningType() )
		{
		case TENSORFLOW    : { do_error("?? unknown"); return false; }
		case DECISION_TREE : ;
		case BAYES         : {
			  cmcVRExtractMLAndHOGFeatureSet naba = new cmcVRExtractMLAndHOGFeatureSet(xMSet,logger);
			  boolean ib = naba.extractImageFeatures();
			  naba = null;
			  return ib;
		}
		case SVM : {
			  cmcVRExtractMLAndHOGFeatureSet naba = new cmcVRExtractMLAndHOGFeatureSet(xMSet,logger);
			  boolean ib = naba.extractHOGFeatures();
			  naba = null;
			  return ib;	
		}
	...
	//-----------------------------------------------------------------------
	private boolean doVRTrainModel()
	//-----------------------------------------------------------------------
	{
		switch( xMSet.getMachineLearningType() )
		{
		case TENSORFLOW : { do_error("?? unknown"); return false; }
		case BAYES : { 
			cmcMachineLearningModelController nav = new cmcMachineLearningModelController(xMSet,logger);
	  		boolean ib = nav.makeAndTrainMLModel( xMSet.getBayesWEKAARFFFileName() , xMSet.getBayesModelName() );
	  	    nav=null;
	  	    return ib;
		}
		case DECISION_TREE : { 
			cmcMachineLearningModelController nav = new cmcMachineLearningModelController(xMSet,logger);
	  		boolean ib = nav.makeAndTrainMLModel( xMSet.getBayesWEKAARFFFileName() , xMSet.getDecisionTreeModelName() );
	  	    nav=null;
	  	    return ib;
		}
		case K_NEAREST_NEIGHBOURS : {
			cmcMachineLearningModelController nav = new cmcMachineLearningModelController(xMSet,logger);
	  		boolean ib = nav.makeAndTrainMLModel( xMSet.getBayesWEKAARFFFileName() , xMSet.getKNNModelName() );
	  	    nav=null;
	  	    return ib;
		}
		case SVM : { do_error("SVM train module WIP"); return false; }
		default : { do_error( "(doVRTrainModel) Unsupported model" + xMSet.getMachineLearningType() ); return false; }
		}
	}
	
	//-----------------------------------------------------------------------
	private boolean doVRMakeAndRunSingleSet()
	//-----------------------------------------------------------------------
	{
		// check whether it is only required to extract the files
		if( xMSet.getAIPostProcessIndicator() == false ) return true;
	
		// allright - you need to run the model also
	    ...
	    case TENSORFLOW : {
			...
			ib = vr.performVisualRecognition();
			if( ib == false ) sMsg = vr.getErrorMessage();
			vr=null;
		    break;
	    }
	    case K_NEAREST_NEIGHBOURS : ;
	    case DECISION_TREE : ;
	    case BAYES : {
	    	cmcVRRunMachineLearning bay = new cmcVRRunMachineLearning(xMSet,logger);
			ib= bay.performVisualRecognition();
			if( ib == false ) sMsg = bay.getErrorMessage();
			bay=null;
            break;
	    }
	    case SVM :
	    default : { do_error ("Make and Run not supported for [" + xMSet.getMachineLearningType() + "]"); ib=false; break; }
	    }
		...
		if( ib == false ) xMSet.setOCRSummaryResult(sMsg==null?"Unknown error":sMsg);
		return ib;
	}
	
	/*
	//-----------------------------------------------------------------------
	private boolean doVRMakeAndRunSingleSetBayes()
	//-----------------------------------------------------------------------
	{
		// check whether it is only required to extract the files
		if( xMSet.getAIPostProcessIndicator() == false ) return true;
		...
		// allright - you need to run the model also
		xMSet.setOCRSummaryResult(null);
		cmcVRRunNaiveBayes bay = new cmcVRRunNaiveBayes(xMSet,logger);
		boolean ib= bay.performVisualRecognition();
		if( ib == false ) xMSet.setOCRSummaryResult(bay.getErrorMessage());
		bay=null;
		return false;
	}

	//-----------------------------------------------------------------------
	private boolean doVRMakeAndRunSingleSetSVM()
	//-----------------------------------------------------------------------
	{
		// check whether it is only required to extract the files
		if( xMSet.getAIPostProcessIndicator() == false ) return true;
		// 
		do_error("svm_make_single_Set must be extended just like tensorflow");
		return false;
	}

	//-----------------------------------------------------------------------
	private boolean doVRMakeAndRunSingleSetTensorflow()
	//-----------------------------------------------------------------------
	{
		// check whether it is only required to extract the files (PostProcessInd = false)
		if( xMSet.getAIPostProcessIndicator() == false ) return true;
		
		//  Besides extracting a single set of images we also need tensorflow to perform visual recognition
		xMSet.setOCRSummaryResult(null);
		cmcVRRunTensorFlow vr = new cmcVRRunTensorFlow(xMSet,logger);
	...
	//-----------------------------------------------------------------------
	private void processGradient(cmcProcEnums.HOG_GRADIENT_DISPLAY_TYPE tipe)
	//-----------------------------------------------------------------------
	{
		cPage.cmcImg.workPixels = null;
		switch(tipe)
		{
		case WIDE : { cPage.cmcImg.workPixels = ibulk.gradientwide( cPage.cmcImg.pixels , cPage.cmcImg.getWidth() ); break; }
		case NARROW : { cPage.cmcImg.workPixels = ibulk.gradientnarrow( cPage.cmcImg.pixels , cPage.cmcImg.getWidth() ); break; }
		case HORIZONTAL_GRADIENT : ;
		case VERTICAL_GRADIENT : ;
		case MAGNITUDE : ;
		case THETA :  {
			 cPage.cmcImg.workPixels = ibulk.DisplayHOGGradient( cPage.cmcImg.pixels , cPage.cmcImg.getWidth() , tipe  );
			 break; }
        default : {
        	do_error( "Unsupported gradient display type [" + tipe + "]");
        	return; }
     	}
		cPage.cmcImg.moveWorkPixelsToImage();
	}
