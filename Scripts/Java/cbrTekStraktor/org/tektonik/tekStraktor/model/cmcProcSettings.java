...
import org.tektonik.tekStraktor.monitor.cmcMonitorDataObjectManager;
import org.tektonik.tekStraktor.project.cbrTekStraktorProjectManager;
import org.tektonik.tekStraktor.project.cmcProjectWrapper;
import org.tektonik.tools.generalpurpose.gpLanguage;
...
public class cmcProcSettings {
	...
	public boolean MLVERSION = false;
	public boolean TENSOR_SIMULATOR = true;
    ...
    public Rectangle submonitframe = new Rectangle(100,100,800,400);
    public Rectangle modelerframe = new Rectangle(100,100,800,400);
    public Rectangle MLPframe = new Rectangle(50,50,1000,400);
    public Rectangle viewerframe = new Rectangle(100,100,800,400);
    ...
    private cmcProcEnums.DIRECTION directionRequested = cmcProcEnums.DIRECTION.NONE;
	//
    ...
    private boolean performAIPostProcess = false;
    private int maxLloydLevel = 16;
    private cmcProcEnums.MACHINE_LEARNING_TYPE MachineLearningType = cmcProcEnums.MACHINE_LEARNING_TYPE.UNKNOWN;
    private String lastARFF=null;
	...
	//---------------------------------------------------------------------------------
	{
		return getApplicationName() + " " + cmcProcConstants.Version + " (" + cmcProcConstants.Build + ")";
	}
	//---------------------------------------------------------------------------------
	...
	public cmcProcEnums.REASSESMENT_COVERAGE getTensorCoverage() { return projman.getTensorCoverage(); }
	public boolean getAlsoRunTesseract() { return projman.getAlsoRunTesseract(); }
	//public boolean getAlsoRunTensorFlow() { return projman.getAlsoRunTensorFlow(); }
	public cmcProcEnums.MACHINE_LEARNING_TYPE getPostProcessAIType() { return projman.getPostProcessAIType(); }
	public String getSplashPictureName() { return projman.getSplashPictureName(); }

	public cmcProcEnums.DIRECTION getDirectionRequested()
	{
		return directionRequested;
	}
	public void setDirectionRequested(cmcProcEnums.DIRECTION dirq)
	{
		directionRequested = dirq;
	}
	...
	public void setAIPostProcessIndicator(boolean ib)
	{
		this.performAIPostProcess = ib;
	}
	public boolean getAIPostProcessIndicator()
	{
		return performAIPostProcess;
	}
	...
	//---------------------------------------------------------------------------------
	{
		...
		ps.println("submonitorlocation=" + (int)submonitframe.getX() + "," + (int)submonitframe.getY() );
		ps.println("submonitorsize=" + (int)submonitframe.getWidth() + "," + (int)submonitframe.getHeight());
		ps.println("modelerlocation=" + (int)modelerframe.getX() + "," + (int)modelerframe.getY() );
		ps.println("modelersize=" + (int)modelerframe.getWidth() + "," + (int)modelerframe.getHeight());
		ps.println("viewerlocation=" + (int)viewerframe.getX() + "," + (int)viewerframe.getY() );
		ps.println("viewerrsize=" + (int)viewerframe.getWidth() + "," + (int)viewerframe.getHeight());
		ps.println("multilayerperceptronlocation=" + (int)MLPframe.getX() + "," + (int)MLPframe.getY() );
		ps.println("multilayerperceptronsize=" + (int)MLPframe.getWidth() + "," + (int)MLPframe.getHeight());
	    //
		...
		ps.println("RecentARFFFile=" + lastARFF );
		ps.println("=============================================================");
		ps.close();
		//
	...
	//---------------------------------------------------------------------------------
	{
		...
		for(int i=0;i<(aantal+1);i++)
		{
			...
			if( sLijn.indexOf("size=") > 0) {
			    ...
			    if( rx != null ) {
			       //do_error( sLijn );
			    	...
			    	if( sLijn.startsWith("submonitor") ) submonitframe = rx;
			    	if( sLijn.startsWith("modeler") ) modelerframe = rx;
			    	if( sLijn.startsWith("multilayerperceptron") ) MLPframe = rx;
			    	if( sLijn.startsWith("viewer") ) viewerframe = rx;
			    }
			...
			if( sLijn.startsWith("RecentARFFFile") ) {
			    sLijn = xU.GetVeld(sLijn,2,'=').trim();
			    if( xU.IsBestand(sLijn) ) {
			    	lastARFF=sLijn;
			    }
			}
		}
	...
	//---------------------------------------------------------------------------------
	{
		...
		do_error( "MONIT" + monitframe.toString() );
	}
	//---------------------------------------------------------------------------------
	public void setSubMonitorFrameBounds(int x , int y , int w , int h)
	//---------------------------------------------------------------------------------
	{
		submonitframe.setBounds(x, y, w, h);
		do_error( "SUBM" + submonitframe.toString() );
	}
	
	public int getMaxLloydLevel()
	{
		return this.maxLloydLevel;
	}
	public void setMaxLloydLevel(int i)
	{
		this.maxLloydLevel = i;
	}
	
	public String getBayesDir()
    {
	     return projman.getBayesDir();
    }
	public String getSandBoxDir()
	{
		return getBayesDir() + xU.ctSlash + "SandBox";
	}
    public String getBayesResultDir()
    {
    	return getBayesDir() + xU.ctSlash  + "model";
    }
    public String getBayesModelName()
    {
    	return getBayesResultDir() + xU.ctSlash  + "BayesModel.xml";
    }
    public String getDecisionTreeModelName()
    {
    	return getBayesResultDir() + xU.ctSlash  + "DecisionTreeModel.xml";
    }
    public String getKNNModelName()
    {
    	return getBayesResultDir() + xU.ctSlash  + "KNearestNeighboursModel.xml";
    }
    public String getWEKADumpFileName()
    {
		 return getBayesDir() + xU.ctSlash + "Weka" + xU.ctSlash + "cbrtekstraktorHOG.arff";
    }
    public String getHOGValidDir()
    {
    	return getBayesDir() + xU.ctSlash  + "validbubble";
    }
    public String getHOGInvalidDir()
    {
    	return getBayesDir() + xU.ctSlash  + "invalidbubble";
    }
    public String getBayesTrainingDirText()
    {
    	return getBayesDir() + xU.ctSlash  + "Text";
    }
    public String getBayesTrainingDirGraph()
    {
    	return getBayesDir() + xU.ctSlash  + "Graph";
    }
    public String getBayesTrainingDirMixed()
    {
    	return getBayesDir() + xU.ctSlash  + "Mixed";
    }
    public String getBayesWEKAProtoTypeARFFFileName()
    {
    	return getBayesResultDir() + xU.ctSlash + "cbrTekstraktorPrototype.arff";
    } 
    public String getBayesWEKAARFFFileName()
    {
    	return getBayesResultDir() + xU.ctSlash + "cbrTekstraktorAll.arff";
    }
    public String getBayesWEKAARFFImageName()
    {
    	return getBayesResultDir() + xU.ctSlash + "cbrTekstraktorAll.png"; 
    }
    public cmcProcEnums.MACHINE_LEARNING_TYPE getMachineLearningType()
    {
    	return MachineLearningType;
    }
	public void setMachineLearningType( cmcProcEnums.MACHINE_LEARNING_TYPE tipe )
	{
		MachineLearningType = tipe;
	}
    public void setLastARFFFileName(String s)
    {
    	lastARFF=s;
    }
    public String getLastARFFFileName()
    {
    	return lastARFF;
    }
    
    
    public String getSandBoxSourceDir()
    {
      	return getSandBoxDir() + xU.ctSlash  + "Source";
    }
    public String getSandBoxTempDir()
    {
      	return getSandBoxDir() + xU.ctSlash  + "Tmp";
    }
    public String getSandBoxTestDir()
    {
      	return getSandBoxDir() + xU.ctSlash  + "Test";
    }
    public String getSandBoxTrainDir()
    {
      	return getSandBoxDir() + xU.ctSlash  + "Train";
    }
    
    public String getEpochWeightDumpFileName()
    {
    	 return getSandBoxTempDir() + xU.ctSlash + "mpl-epoch-weights.txt";
    }
    public String getPreviousEpochWeightDumpFileName()
    {
    	 return getSandBoxTempDir() + xU.ctSlash + "mpl-prev-epoch-weights.txt";
    }
    public String getEpochStatXMLFileName()
    {
    	 return getSandBoxTempDir() + xU.ctSlash + "mpl-epoch-stats.xml";
    }
    public String getPreviousEpochStatXMLFileName()
    {
    	 return getSandBoxTempDir() + xU.ctSlash + "mpl-prev-epoch-stats.xml";
    }
    public String getPreviousModelFileName()
    {
    	 return getSandBoxTempDir() + xU.ctSlash + "mpl-prev-model.xml";
    }
    public String getEpochPictureFileName()
    {
    	 return getSandBoxTempDir() + xU.ctSlash + "mpl-epoch-stats.png";
    }
    public String getEpochROCImageFileName()
    {
    	 return getSandBoxTempDir() + xU.ctSlash + "mpl-epoch-roc.png";
    }
    public String getEpochInterruptFileName()
    {
    	 return getSandBoxTempDir() + xU.ctSlash + "mpl-epoch-interrupt.txt";
    }
    public String getEpochDebugFileName()
    {
    	 return getSandBoxTempDir() + xU.ctSlash + "mpl-epoch-debug.txt";
    }
    
    public String getARFFFileNameTrainingSet(String OriginalFileName)
    {
    	String ARFFile = xU.getFolderOrFileName( OriginalFileName );
    	if( ARFFile == null ) { do_error("Cannot extract ARFFFile name"); return null; }
        return getSandBoxTrainDir() + xU.ctSlash + ARFFile;
    }
    public String getARFFFileNameTestSet(String OriginalFileName)
    {
    	String ARFFile = xU.getFolderOrFileName( OriginalFileName );
    	if( ARFFile == null ) { do_error("Cannot extract ARFFFile name"); return null; }
        return getSandBoxTestDir() + xU.ctSlash + ARFFile;
    }
    public String getARFFMemoryMappedTrainFileName(String OriginalFileName)
    {
    	String ARFFile = xU.getFolderOrFileName( OriginalFileName );
    	if( ARFFile == null ) { do_error("Cannot extract ARFFFile name"); return null; }
    	String NoSuffix = xU.getFileNameWithoutSuffix( ARFFile );
    	if( NoSuffix == null ) { do_error("Cannot strip suffix"); return null; }
    	return getSandBoxTempDir() + xU.ctSlash + NoSuffix + "-train-mem.txt";
    }
    public String getARFFMemoryMappedTestFileName(String OriginalFileName)
    {
    	String ARFFile = xU.getFolderOrFileName( OriginalFileName );
    	if( ARFFile == null ) { do_error("Cannot extract ARFFFile name"); return null; }
    	String NoSuffix = xU.getFileNameWithoutSuffix( ARFFile );
    	if( NoSuffix == null ) { do_error("Cannot strip suffix"); return null; }
    	return getSandBoxTempDir() + xU.ctSlash + NoSuffix + "-test-mem.txt";
    }
