...
public class cmcMonitorDialog {
	...
	private boolean isSubMonitor=false;
	...
	//---------------------------------------------------------------------------------
	{
		...
		//
		isSubMonitor = xMSet.getmoma().getSubMonitorActive();  // local var because it changes before window closes
		initialize(jf);
	}
    ...
    //-----------------------------------------------------------------------
  	...
  	//-----------------------------------------------------------------------
  	{
	  	    String sTitle =  isSubMonitor ? "TensorFlow reassessment" : "Bulk Processor Monitor";
	  	    sTitle += " " + blkFolderName;
	  		try {
	                ...
	                //	            
	  	    		...
	  	    		*/
	  	            ...
	  	            dialog.setLocationByPlatform(true);
	  		  		if( isSubMonitor ) dialog.setBounds( xMSet.submonitframe );
	  		  		              else dialog.setBounds( xMSet.monitframe );
	    ...
	    //-----------------------------------------------------------------------
	    private void perform_move( JDialog diag)
	    //-----------------------------------------------------------------------
	    {
	      keepdimension(diag);
	    }
	    private void keepdimension(JDialog diag)
	    {
	    	 do_log( 9 , "===>" + diag.getX() + " " + diag.getY() + diag.getWidth() + " " + diag.getHeight() );
	    	 if( isSubMonitor ) xMSet.setSubMonitorFrameBounds( diag.getX() , diag.getY() , diag.getWidth() , diag.getHeight() );
	              else xMSet.setMonitorFrameBounds( diag.getX() , diag.getY() , diag.getWidth() , diag.getHeight() );
	    }
		//---------------------------------------------------------------------------------
