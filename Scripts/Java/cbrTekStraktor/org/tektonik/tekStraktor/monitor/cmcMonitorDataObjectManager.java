package org.tektonik.tekStraktor.monitor;
...
import org.tektonik.tekStraktor.featureExtraction.cmcVRArchive;
import org.tektonik.tekStraktor.featureExtraction.cmcVRMakeTrainingImages;
import org.tektonik.tekStraktor.featureExtraction.cmcVRParagraph;
...
public class cmcMonitorDataObjectManager {
	...
	private boolean subMonitorActive=false;
	...
	//---------------------------------------------------------------------------------
	public boolean createVRParagraphCheckList(String LongFName )
	//---------------------------------------------------------------------------------
	{
		    creatime=System.currentTimeMillis();
			scanList=null;
	    	...
	    	int image_width  = -1;
	    	int image_heigth = -1;
	    	boolean MakeHogDescriptor=false;
	    	...
	    	case TENSORFLOW    : {
	     		image_width = cmcProcConstants.VR_TENSOR_IMAGE_WIDTH;
	    		image_heigth = cmcProcConstants.VR_TENSOR_IMAGE_HEIGTH;
	    		MakeHogDescriptor=false;
	            break; 		
	    	}
	    	case SVM : {
	    		image_width = cmcProcConstants.VR_HOG_IMAGE_WIDTH;
	    		image_heigth = cmcProcConstants.VR_HOG_IMAGE_HEIGTH;
	    		MakeHogDescriptor=true;
                break;
	    	}
	    	default : { do_error("(cmcMonitorDataObject) Unsupported type [" + xMSet.getMachineLearningType() + "]"); return false; }
	    	}
	    	if( (image_heigth < 0) || (image_width < 0) ) {
	    		do_error("Could not get the requried size of the image");
	    		return false;
	    	}
	    	//
			cmcVRMakeTrainingImages ms = new cmcVRMakeTrainingImages( xMSet , logger , image_width , image_heigth , MakeHogDescriptor );
	    	...
	    	if( ib ) {
              ...
              // do we also need to process the images extracted ?
	      	  if( xMSet.getAIPostProcessIndicator() ) {
	      		  scanList = null;   // 
	      		  scanList = new ArrayList<cmcMonitorItem>();
...
//do_error( scanList.get(i).getFileName() + " " + scanList.get(i).getStarttime() );
	...
	//---------------------------------------------------------------------------------
	{
		...
		scanListBackup=null;
		subMonitorActive=false;
    }

	//---------------------------------------------------------------------------------
	public boolean getSubMonitorActive()
	//---------------------------------------------------------------------------------
	{
			return subMonitorActive;
	}
