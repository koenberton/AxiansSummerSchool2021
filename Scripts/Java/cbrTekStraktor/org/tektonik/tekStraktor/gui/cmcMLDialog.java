...
import org.tektonik.MachineLearning.LogisticRegression.cmcLinearRegression;
import org.tektonik.MachineLearning.LogisticRegression.cmcLogisticRegression;
import org.tektonik.MachineLearning.SVM.cmcSMO;
import org.tektonik.MachineLearning.SVM.cmcSMODTO;
import org.tektonik.MachineLearning.SVM.cmcSMODTOCore;
...
public class cmcMLDialog {
	...
	class MLDTO {
		...
		// specific for SMO
		int SMO_Cycles = cmcMachineLearningConstants.SMO_NUMBER_OF_CYCLES;
		double SMO_SoftMargin = cmcMachineLearningConstants.SMO_SOFTMARGIN;
		double SMO_Tolerance = cmcMachineLearningConstants.SMO_TOLERANCE;
		double SMO_GaussianSigma = cmcMachineLearningConstants.SMO_GAUSSIAN_SIGMA;
		double SMO_RBFGamma = cmcMachineLearningConstants.SMO_RBF_GAMMA;
		cmcMachineLearningEnums.KernelType SMO_KernelTipe = cmcMachineLearningEnums.KernelType.NONE;
		//
		String comment=null;
	...
	*/
	...
	//------------------------------------------------------------
	{
		...
		txtComment.setBounds( txtComment.getX() , txtComment.getY() , dwidth - (4 * XMARGIN) - txtComment.getX() , txtComment.getHeight());
	...
	//------------------------------------------------------------
	{
		...
		JLabel lblRelation = new JLabel("Relation Name");
		...
		lblComment.setFont(dfont);
	    lblComment.setBounds( x , y , LABEL_WIDTH , WIDGET_HEIGTH );
	    p.add(lblComment);
		//
	    //
		y += WIDGET_HEIGTH + VERTICAL_GAP;
		JLabel lblLines = new JLabel("Rows");
		...
		//
		y += WIDGET_HEIGTH + VERTICAL_GAP;
		txtComment = new JTextField( mldto.comment==null ? "" : mldto.comment );
		txtComment.setBounds( x , y , LABEL_WIDTH * 2, WIDGET_HEIGTH );
		txtComment.setFont(dfont);
		...
		txtNbrOfLines.setEditable(false);
		p.add(txtNbrOfLines);
		//
		y += WIDGET_HEIGTH + 2;
	...
	//------------------------------------------------------------
	{
		   ...
		   if( xMSet.xU.IsBestand( fname ) == false ) return;
	 	   cmcARFFDAOLight dap = new cmcARFFDAOLight( xMSet , logger );
	       boolean ib = dap.performFirstPass( fname , "makeMLDTO" );
	       if ( ib == false ) {
	       ...
	       mldto.featureList      = dap.getFeatureList();
	       mldto.firstNonString   = dap.getFirstNonString();
	       mldto.comment          = dap.getRelationComment();
	       mldto.fullcomment      = dap.getFullCommentHTMLString();
	       //
	       xMSet.setLastARFFFileName(fname);
	       visualizer.setBackGroundColor(btnT1Browse.getBackground());
//visualizer.setBackGroundColor(Color.DARK_GRAY);
	...
	//------------------------------------------------------------
	{
		 ...
		 txtComment.setText( mldto.comment==null ? "" : mldto.comment.trim() );
		 txtComment.setToolTipText( mldto.fullcomment == null ? "no information" : mldto.fullcomment );
		 ...
		 txtTrainPerc.setText( ""+cmcMachineLearningConstants.TEST_SET_PERCENTAGE + "%" );
