...
import org.tektonik.MachineLearning.ARFF.ARFFVisualizer;
...
import org.tektonik.MachineLearning.SVM.cmcPerceptron;
import org.tektonik.tekStraktor.dao.cmcArchiveDAO;
import org.tektonik.tekStraktor.dao.cmcBookMetaDataDAO;
import org.tektonik.tekStraktor.drawing.cmcGraphController;
import org.tektonik.tekStraktor.model.cmcDrawHistogram;
import org.tektonik.tekStraktor.model.cmcProcConstants;
import org.tektonik.tekStraktor.model.cmcProcController;
import org.tektonik.tekStraktor.model.cmcProcEnums;
import org.tektonik.tekStraktor.model.cmcProcSemaphore;
import org.tektonik.tekStraktor.model.cmcProcSettings;
import org.tektonik.tekStraktor.model.cmcProcTimingInfo;
import org.tektonik.tekStraktor.model.comicPage;
import org.tektonik.tekStraktor.monitor.cmcMonitorController;
import org.tektonik.tekStraktor.ocr.cmcOCRController;
import org.tektonik.tools.generalGUIPurpose.widgets.cmcMetaDataPanel;
import org.tektonik.tools.generalGUIPurpose.windows.cmcFileViewer;
import org.tektonik.tools.generalGUIPurpose.windows.cmcProcFileChooser;
import org.tektonik.tools.generalImagePurpose.cmcImagePrimitives;
import org.tektonik.tools.generalImagePurpose.gpMakeScreenShot;
import org.tektonik.tools.generalImagePurpose.gpRotateImageFile;
...
import org.tektonik.tools.logger.nioServerDaemon;
...
public class comicImageProcessorMainGUI {
    ...
    static cmcMenuManagerML menuManML=null;
    ...
    private org.tektonik.tekStraktor.model.cmcProcEnums.APPLICATION_STATE appState = org.tektonik.tekStraktor.model.cmcProcEnums.APPLICATION_STATE.IDLE; 
    private org.tektonik.tekStraktor.model.cmcProcEnums.APPLICATION_STATE prevAppState = org.tektonik.tekStraktor.model.cmcProcEnums.APPLICATION_STATE.IMAGE; // something different
	...
	private void initialize() {
	    ...
	    if( !xMSet.MLVERSION ) {
	      ...
	      menuManML = new cmcMenuManagerML(menuShared);
	  	  menuManML.makeMenus(menuBar,xfont,LEFTPANELWIDTH,arFileStack);
	    }
	    //
		...
		if( xMSet.MLVERSION ) {
			...
			btnExtract.setText("Machine Learning");
			btnEdit.setText("Neural Network");
			btnOCR.setText("Clustering");
			     ...
			     if( sema != cmcProcSemaphore.TaskType.DO_NOTHING) {
			    	 ...
			    	 requestHistogram=true;
			     	 ...
			     	 if( robotMode ) {
			     		 ...
			     		 case DO_DIALOG     : { if ( toon_dialoog() == false ) {runCompleted=true; enable_controls(org.tektonik.tekStraktor.model.cmcProcEnums.APPLICATION_STATE.IDLE); } else doe_preprocess(); break; }
			     		 ...
			     		 //
			     		 case PREPARE_OCR   : { enable_controls(org.tektonik.tekStraktor.model.cmcProcEnums.APPLICATION_STATE.OCR_BUSY); doe_show_ocr_file(); break; }
			     		 case EDIT_OCR_FILE : { doe_tesseract(); break; }
			     		 case SHOW_OCR_FILE : { ask_tesseractoption(); break; }
			     		 case RUN_TESSERACT : { process_ocr_result(); runCompleted=true; syncMonitorEnd(); enable_controls(org.tektonik.tekStraktor.model.cmcProcEnums.APPLICATION_STATE.IDLE); cleanBulkEnvironment(); break; }
			     		 //
			     		...
			     		switch (sema )
			     		 {
			     		 case SHOUT              : { do_shout(); break; }
			     		 case DO_LOAD_IMAGE      : { enable_controls(org.tektonik.tekStraktor.model.cmcProcEnums.APPLICATION_STATE.IMAGE); toonStatusInfo(cPage.getShortImageInfo()); break; }
			     		 
			     		 case PROCESS_GRAYSCALE  : { enable_controls(org.tektonik.tekStraktor.model.cmcProcEnums.APPLICATION_STATE.IMAGE); break; }
			     		 case PROCESS_BLACKWHITE : { enable_controls(org.tektonik.tekStraktor.model.cmcProcEnums.APPLICATION_STATE.IMAGE); break; }
			     		 case PROCESS_NIBLAK     : { enable_controls(org.tektonik.tekStraktor.model.cmcProcEnums.APPLICATION_STATE.IMAGE); break; }
			     		 case PROCESS_SAUVOLA    : { enable_controls(org.tektonik.tekStraktor.model.cmcProcEnums.APPLICATION_STATE.IMAGE); break; }
			     		 case PROCESS_INVERT     : { enable_controls(org.tektonik.tekStraktor.model.cmcProcEnums.APPLICATION_STATE.IMAGE); break; }
			     		 case PROCESS_STATS      : { enable_controls(org.tektonik.tekStraktor.model.cmcProcEnums.APPLICATION_STATE.IMAGE); break; }
			     		 case PROCESS_BLEACH     : { enable_controls(org.tektonik.tekStraktor.model.cmcProcEnums.APPLICATION_STATE.IMAGE); break; }
			     		 case PROCESS_EQUAL      : { enable_controls(org.tektonik.tekStraktor.model.cmcProcEnums.APPLICATION_STATE.IMAGE); break; }
			     		 case PROCESS_MAINFRAME     : { enable_controls(org.tektonik.tekStraktor.model.cmcProcEnums.APPLICATION_STATE.IMAGE); break; }
			     		 case PROCESS_BLUEPRINT     : { enable_controls(org.tektonik.tekStraktor.model.cmcProcEnums.APPLICATION_STATE.IMAGE); break; }
			     		 case PROCESS_CONVOLUTION_EDGE     : { enable_controls(org.tektonik.tekStraktor.model.cmcProcEnums.APPLICATION_STATE.IMAGE); break; }
			     		 case PROCESS_CONVOLUTION_GLOW     : { enable_controls(org.tektonik.tekStraktor.model.cmcProcEnums.APPLICATION_STATE.IMAGE); break; }
			     		 case PROCESS_CONVOLUTION_SHARPEN  : { enable_controls(org.tektonik.tekStraktor.model.cmcProcEnums.APPLICATION_STATE.IMAGE); break; }
			     		 case PROCESS_CONVOLUTION_GAUSSIAN : { enable_controls(org.tektonik.tekStraktor.model.cmcProcEnums.APPLICATION_STATE.IMAGE); break; }
			     		 case PROCESS_SOBELGRAYSCALE  : { enable_controls(org.tektonik.tekStraktor.model.cmcProcEnums.APPLICATION_STATE.IMAGE); break; }
			     		 case PROCESS_SOBELCOLOR  : { enable_controls(org.tektonik.tekStraktor.model.cmcProcEnums.APPLICATION_STATE.IMAGE); break; }
			     		 case PROCESS_GRADIENT_NARROW  : { enable_controls(org.tektonik.tekStraktor.model.cmcProcEnums.APPLICATION_STATE.IMAGE); break; }
			     		 case PROCESS_GRADIENT_WIDE  : { enable_controls(org.tektonik.tekStraktor.model.cmcProcEnums.APPLICATION_STATE.IMAGE); break; }
			     		 case PROCESS_GRADIENT_MAGNITUDE  : { enable_controls(org.tektonik.tekStraktor.model.cmcProcEnums.APPLICATION_STATE.IMAGE); break; }
			    		 case PROCESS_GRADIENT_THETA  : { enable_controls(org.tektonik.tekStraktor.model.cmcProcEnums.APPLICATION_STATE.IMAGE); break; }
			    		 case PROCESS_VERTICAL_GRADIENT  : { enable_controls(org.tektonik.tekStraktor.model.cmcProcEnums.APPLICATION_STATE.IMAGE); break; }
			    		 case PROCESS_HORIZONTAL_GRADIENT  : { enable_controls(org.tektonik.tekStraktor.model.cmcProcEnums.APPLICATION_STATE.IMAGE); break; }
					     case PROCESS_CANNY     : { enable_controls(org.tektonik.tekStraktor.model.cmcProcEnums.APPLICATION_STATE.IMAGE); break; }
			    		 case PROCESS_TEXTURE     : { enable_controls(org.tektonik.tekStraktor.model.cmcProcEnums.APPLICATION_STATE.IMAGE); break; }
			    		 case PROCESS_MAXLLOYD     : { enable_controls(org.tektonik.tekStraktor.model.cmcProcEnums.APPLICATION_STATE.IMAGE); break; }
			     		 
			    		 
			    		 case DO_EDITOR_IMAGE    : { doe_fastload( gCon.getWorkImageFile()); enable_controls(org.tektonik.tekStraktor.model.cmcProcEnums.APPLICATION_STATE.EDIT); break; }
			     		 case DO_FAST_LOAD_IMAGE : { toonStatusInfo(gCon.getShortPageInfo()); disable_controls(org.tektonik.tekStraktor.model.cmcProcEnums.APPLICATION_STATE.EDIT); 
			     		                             //btnEdit.setEnabled(true);
			     		                             setControlStatusW("EDIT",true);
			     		                             break; }
			     		 case DO_FLUSH_CHANGES   : { enable_controls(org.tektonik.tekStraktor.model.cmcProcEnums.APPLICATION_STATE.IDLE);
        ...
        if( xMSet.isActive() == false ) {
        	...
        	disable_controls(org.tektonik.tekStraktor.model.cmcProcEnums.APPLICATION_STATE.IDLE);
        	// Fixit
	...
	//-----------------------------------------------------------------------
	{
			...
			//
			disable_controls(org.tektonik.tekStraktor.model.cmcProcEnums.APPLICATION_STATE.IMAGELOADING);
			//
	}
	...
	//-----------------------------------------------------------------------
	{
	    ...
	    if( (targetlevel < 2) || (targetlevel > 255) ) {
	    	...
	    	enable_controls(org.tektonik.tekStraktor.model.cmcProcEnums.APPLICATION_STATE.IMAGE);
        	return;
	    }
	...
	//-----------------------------------------------------------------------
	{
		if( xMSet.MLVERSION ) return;
		org.tektonik.tekStraktor.model.cmcProcEnums.MENU_ITEMS_STD mi = null;
		org.tektonik.tekStraktor.model.cmcProcEnums.POP_ITEMS pi = null;
		//
		if( control.toUpperCase().startsWith("IMAGE") ) {
			btnImage.setEnabled(stat);
			mi = org.tektonik.tekStraktor.model.cmcProcEnums.MENU_ITEMS_STD.LOAD_IMAGE;
			pi = org.tektonik.tekStraktor.model.cmcProcEnums.POP_ITEMS.LOAD_IMAGE;
		}
		else
		if( control.toUpperCase().startsWith("EXTRACT") ) {
			btnExtract.setEnabled(stat);
			mi = org.tektonik.tekStraktor.model.cmcProcEnums.MENU_ITEMS_STD.EXTRACT_TEXT;
			pi = org.tektonik.tekStraktor.model.cmcProcEnums.POP_ITEMS.EXTRACT_TEXT;
		}
		else
		if( control.toUpperCase().startsWith("EDIT") ) {
			btnEdit.setEnabled(stat);
			mi = org.tektonik.tekStraktor.model.cmcProcEnums.MENU_ITEMS_STD.START_EDIT;
			pi = org.tektonik.tekStraktor.model.cmcProcEnums.POP_ITEMS.STOP_EDIT;
		}
		else
		if( control.toUpperCase().startsWith("OCR") ) {
			btnOCR.setEnabled(stat);
			mi = org.tektonik.tekStraktor.model.cmcProcEnums.MENU_ITEMS_STD.OCR;
			pi = org.tektonik.tekStraktor.model.cmcProcEnums.POP_ITEMS.OCR;
		}
		else
		if( control.toUpperCase().startsWith("TRANS") ) {
			btnTranslate.setEnabled(stat);
			mi = org.tektonik.tekStraktor.model.cmcProcEnums.MENU_ITEMS_STD.TRANSLATE;
			pi = org.tektonik.tekStraktor.model.cmcProcEnums.POP_ITEMS.TRANSLATE;
		}
		else
		if( control.toUpperCase().startsWith("REPORT") ) {
			btnReport.setEnabled(stat);
			mi = org.tektonik.tekStraktor.model.cmcProcEnums.MENU_ITEMS_STD.REPORT;
			pi = org.tektonik.tekStraktor.model.cmcProcEnums.POP_ITEMS.REPORT;
		}
		else
		if( control.toUpperCase().startsWith("STAT") ) {
			btnStats.setEnabled(stat);
			mi = org.tektonik.tekStraktor.model.cmcProcEnums.MENU_ITEMS_STD.STATISTICS;
			pi = org.tektonik.tekStraktor.model.cmcProcEnums.POP_ITEMS.STATISTICS;
		}
		else
	...
	//-----------------------------------------------------------------------
	private void disable_controls(org.tektonik.tekStraktor.model.cmcProcEnums.APPLICATION_STATE as)
	//-----------------------------------------------------------------------
	{
	...
	//-----------------------------------------------------------------------
	private void enable_controls(org.tektonik.tekStraktor.model.cmcProcEnums.APPLICATION_STATE as)
	//-----------------------------------------------------------------------
	{
	...
	//-----------------------------------------------------------------------
	{
		disable_controls(org.tektonik.tekStraktor.model.cmcProcEnums.APPLICATION_STATE.EDITLOADING);
		pbar.setVisible(true);
		//btnEdit.setEnabled(true);
		setControlStatusW("EDIT",true);
	}
	...
	//-----------------------------------------------------------------------
	private void setQuickButtons(org.tektonik.tekStraktor.model.cmcProcEnums.APPLICATION_STATE ia)
	//-----------------------------------------------------------------------
	{
	//do_error("STATUS -> " + ia );
		...
		//
		if( menuMan != null ) menuMan.setMenuAppState( appState );
		if( popMan != null ) popMan.setMenuAppState( appState );
		//
		if( ia == org.tektonik.tekStraktor.model.cmcProcEnums.APPLICATION_STATE.EDIT ) {
	...
	{
		if( xMSet.MLVERSION ) {
			cmcMultiLayerPerceptronDialog mp = new cmcMultiLayerPerceptronDialog( this.frame , xMSet , logger );
			return;
		}
		clickOnEdit(null);
	}
	...
	//-----------------------------------------------------------------------
	{
		...
		switchToImageMode(false);
		disable_controls(org.tektonik.tekStraktor.model.cmcProcEnums.APPLICATION_STATE.STATSGATHERING);
		process_stats();
	}
	//-----------------------------------------------------------------------
    ...
    //-----------------------------------------------------------------------
  	...
  	//-----------------------------------------------------------------------
  	{
  	    ...
  	    if( xMSet.getAlsoRunTesseract() == false ) { // End
  	      syncMonitorEnd();
  	      runCompleted=true; 
		  enable_controls(org.tektonik.tekStraktor.model.cmcProcEnums.APPLICATION_STATE.EXTRACTED); 
   ...
   //-----------------------------------------------------------------------
   {
    ...
    //-----------------------------------------------------------------------
  	...
  	{
  		//   cmcSMODTO smodto = new cmcSMODTO( OriginalFileName , DataTargetTuple[0].getNbrOfRows() , 0.6 , 0.001 , 40 , params.getKernelTrickTipe() );
        /*
  		cmcSMODTOCore params = new cmcSMODTO( "C:\\Temp\\cbrTekStraktor\\Tutorial\\MachineLearning\\SandBox\\SMO-Python.arff" , cmcMachineLearningEnums.KernelType.NONE );
  		params.setC(0.6);
  		params.setMaxCycles(40);
  		params.setTolerance(0.001);
  		
  		cmcSMO smo = new cmcSMO( xMSet , logger );
  	    smo.testSMO( params.getLongARFFFileName() , "C:\\Temp\\cbrTekStraktor\\Tutorial\\MachineLearning\\SandBox\\junk.xml" , params );
  		*/
  		
  		/*
  		//String ARFF = "C:\\Temp\\cbrTekStraktor\\Tutorial\\MachineLearning\\SandBox\\MLP-Test.arff";
  	    //String ARFF = "C:\\Temp\\cbrTekStraktor\\Tutorial\\MachineLearning\\SandBox\\00-MLP-IRIS.arff";
  	    //String ARFF = "C:\\Temp\\cbrTekStraktor\\Tutorial\\MachineLearning\\SandBox\\00-MLP-ABALONE.arff";
  		String ARFF = "C:\\Temp\\cbrTekStraktor\\Tutorial\\MachineLearning\\SandBox\\cbrtekstraktorall.arff";
  		//String ARFF = "C:\\Temp\\cbrTekStraktor\\Tutorial\\MachineLearning\\SandBox\\00-US-Census-MLP.arff";
  	    //String ARFF = "C:\\Temp\\cbrTekStraktor\\Tutorial\\MachineLearning\\SandBox\\PimaIndiansDiabetes.arff";
  		//String ARFF = "C:\\Temp\\cbrTekStraktor\\Tutorial\\MachineLearning\\SandBox\\00-redwine.arff";
  		//String ARFF = "C:\\Temp\\cbrTekStraktor\\Tutorial\\MachineLearning\\SandBox\\00-italianwine.arff";
  	    //String ARFF = "C:\\Temp\\cbrTekStraktor\\Tutorial\\MachineLearning\\SandBox\\HousingPricesPredict.arff";
  		cmcMLPDTOCore core = new cmcMLPDTOCore( ARFF  );
  	    core.setActivationFunction( neuralLayer.ACTIVATION_FUNCTION_TYPE.TANH );
  		//core.setActivationFunction( neuralLayer.ACTIVATION_FUNCTION_TYPE.HYPERBOLIC_TANGENT );
  		//core.setActivationFunction( neuralLayer.ACTIVATION_FUNCTION_TYPE.SIGMOID );
  		//core.setActivationFunction( neuralLayer.ACTIVATION_FUNCTION_TYPE.RELU );
  		core.setCostFunctionType( neuralLayer.COST_FUNCTION_TYPE.SQUARED_ERROR );
  		core.setNbrOfHiddenLayers( 2 );
  		core.setNbrOfNeuronsPerHiddenLayer( 64 );
  		core.setSizeOfMiniBatch( 5000 );
  		core.setMaximumNumberOfEpochs( 2000 );
  		core.setLearningRate( 0.5 );  // increase learning rate eve to 5 or 10 for the simple sets
  		
  		cmcMLP mpl = new cmcMLP( xMSet , logger );
  		if(ARFF.toUpperCase().endsWith("MLP-TEST.ARFF") ) {
  			do_error( "CREATING TEST SET");
  			mpl.makeTestSet( ARFF );
  		}
  		boolean ib = mpl.testMLP( "C:\\Temp\\cbrTekStraktor\\Tutorial\\MachineLearning\\SandBox\\junk.xml" , core );
  	...
  	private void doClustering()
  	{
	   ...
	   cmcProcFileChooser fc = new cmcProcFileChooser(xMSet.getSandBoxSourceDir() ,false);
	   ARFFFileName = fc.getAbsoluteFilePath();
	   if( ARFFFileName == null )  return;
       if( xMSet.xU.IsBestand(ARFFFileName) == false ) return;
	   //
	   JTextField jperc = new JTextField(10); 
	   jperc.setText("3");
	   Object[] msg = { "Number of clusters" , jperc };
	   ...
	   switch (answer) {
          case JOptionPane.YES_OPTION: {
        	...
        	if( !Double.isNaN(dd)) {
         		KMeans kk = new KMeans( xMSet , logger );
         		kk.trainMLP( ARFFFileName , (int)dd );
        	}
        	break;
          }  //  
