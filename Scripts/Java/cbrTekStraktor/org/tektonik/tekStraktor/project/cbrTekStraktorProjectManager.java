...
public class cbrTekStraktorProjectManager {
    ...
    public String getBayesDir()       { return currProj.getBayesDir(); }
    public String getSplashPictureName()     { return currProj.getSplashPictureName(); }
    //
    ...
    public cmcProcEnums.REASSESMENT_COVERAGE getTensorCoverage() { return currProj.getTensorCoverage(); }
    public boolean getAlsoRunTesseract()                    { return currProj.getAlsoRunTesseract(); }
    //public boolean getAlsoRunTensorFlow()                 { return currProj.getAlsoRunTensorFlow(); }
    public cmcProcEnums.MACHINE_LEARNING_TYPE getPostProcessAIType()     { return currProj.getPostProcessAIType(); }
    //
	...
	//---------------------------------------------------------------------------------
	{
			...
			do_error("Usage : " + xMSet.getApplicationName() + " -D <WorkFolderName> -ML");
	}
			...
			for(int i=0;i<args.length;i++)
			{
				...
				if( args[i].compareToIgnoreCase("-D") == 0) {
					...
					if ( i < args.length ) {
				    	...
				    	sSuperDir = xMSet.xU.getParentFolderName(args[i]);
				    	sRootDir = args[i];
					}
				}
				// Machine learning option
				else if( args[i].compareToIgnoreCase("-ML") == 0) {
					xMSet.MLVERSION = true;
				}
				else {
					Usage();
