package org.tektonik.tekStraktor.featureExtraction;

import java.util.ArrayList;
import java.util.Random;

import org.tektonik.tekStraktor.dao.cmcGraphPageDAO;
...
import org.tektonik.tools.generalImagePurpose.gpLoadImageInBuffer;
...
public class cmcVRMakeTrainingImages {
	...
	private static int vr_image_heigth = -1;
	private static int vr_image_width = -1;
	private boolean makeHogDescriptor=false;
	...
	//------------------------------------------------------------
	public cmcVRMakeTrainingImages(cmcProcSettings is,logLiason ilog , int width , int heigth , boolean makeHog)
	//------------------------------------------------------------
	{
		xMSet = is;
		logger=ilog;
		...
		vr_image_width = width;
		vr_image_heigth = heigth;
		makeHogDescriptor = makeHog;
	}
    ...
    */
	...
	//------------------------------------------------------------
	{
	  	...
	  	if( paraWidth > vr_image_width ) {
	  		MinX = (paraWidth - vr_image_width) / 2;
	  		paraWidth = vr_image_width;
	  	}
	  	if( paraHeigth > vr_image_heigth ) {
	  		MinY = (paraHeigth - vr_image_heigth) / 2;
	  		paraHeigth = vr_image_heigth;
	  	}
	  	//
	  	...
	  	try {
	  	  ...
	  	  // resize to 299x299
	  	  int xmargin = (paraWidth < vr_image_width  ) ? ( vr_image_width - paraWidth ) / 2  : 0;
	  	  int ymargin = (paraHeigth < vr_image_heigth) ? ( vr_image_heigth - paraHeigth ) / 2 : 0;
	  	  curated = new int[ vr_image_width * vr_image_heigth ];
	  	  for(int i=0;i<curated.length;i++) curated[i] = 0x00ffffff;
	  	  z=0;
	  	  for(int i=0;i<paraHeigth;i++)
	  	  {
	  		  int y = i * paraWidth;
	  		  z = ((ymargin + i) * vr_image_width ) + xmargin; 
	  		  for(int j=0;j<paraWidth;j++)
	  		  {
	  			  int k = y + j;
	  			  curated[ z ] = extract[k];
	    ...
	    String OutputFileName = DestDir + xMSet.xU.ctSlash + prefix + sPUID + "_" + pabo.UID + "-" + String.format("%04d", paraWidth ) + "x" + String.format("%04d", paraHeigth ) + ".jpg";
	    ...
	    irout.writePixelsToFile( curated , vr_image_width , vr_image_heigth , OutputFileName , cmcImageRoutines.ImageType.RGB );
		...
		do_log( 9 , "Created [" + OutputFileName + "]");
	  	//    
	    // if this is needed create the HOG descriptor 
	 	boolean ib = makeHogDescriptor ? hogger.makeHOGDescriptor( curated , vr_image_width , OutputFileName , xMSet.getCorpusDir() + xMSet.xU.ctSlash + "Images") : true;
		...
		curated=null;
		//
		return (ib == false) ? null : OutputFileName;
	}
