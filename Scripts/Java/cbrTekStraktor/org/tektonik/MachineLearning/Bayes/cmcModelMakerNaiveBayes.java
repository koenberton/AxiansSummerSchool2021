package org.tektonik.MachineLearning.Bayes;
...
import org.tektonik.MachineLearning.ARFF.cmcARFFRoutines;
...
public class cmcModelMakerNaiveBayes implements cmcModelMaker {
    ...
    //------------------------------------------------------------
    {
     ...
     //------------------------------------------------------------
     {
     	...
     	if( (cat.getTipe() == ARFF_TYPE.NOMINAL) || (cat.getTipe() == ARFF_TYPE.CLASS) )
     	{
   ...
   //------------------------------------------------------------
    ...
    //------------------------------------------------------------
    {
        ...
        {
        	if( categories[i].getTipe() == ARFF_TYPE.NUMERIC ) nums++;
        	if( categories[i].getTipe() == ARFF_TYPE.NOMINAL ) nums++;
        }	
        if( nums < 0 ) {
        ...
        for(int i=0 ; i<categories.length; i++)
        {
        	if( (categories[i].getTipe() == ARFF_TYPE.NUMERIC) ||
        		(categories[i].getTipe() == ARFF_TYPE.CLASS) ||
        		(categories[i].getTipe() == ARFF_TYPE.NOMINAL) ) 
        	{
    ...
    //------------------------------------------------------------
    {
    	...
    	for(int i=0;i<xa.getAttributeList().length;i++)
    	{
    		...
    		if( attr.getTipe() != ARFF_TYPE.NUMERIC ) continue;
    ...
    //------------------------------------------------------------
    {
		...
		for(int col=0 ; col < xa.getAttributeList().length ; col++ )
		{
		   ...
		   if( attr.getTipe() == ARFF_TYPE.STRING ) continue;  
		   if( attr.getTipe() == ARFF_TYPE.CLASS ) continue;  // do not use the class to calculate class .. 
    ...
    //------------------------------------------------------------
    {
        ...
        for(int row=0;row<ndatarows;row++)
    	{
    		...
    		for(int col=0 ; col < xa.getAttributeList().length ; col++ )
    		{
    			...
    			if( (xa.getAttributeList()[col].getTipe() == ARFF_TYPE.STRING) && (FileName==null) ) {
    				FileName = xa.getAttributeList()[col].getSValue( row );
    			}
       		    // only accept NOMINAL or NUMERIC
       		    if( xa.getAttributeList()[col].getTipe() == ARFF_TYPE.CLASS ) continue;  
       		    if( xa.getAttributeList()[col].getTipe() == ARFF_TYPE.STRING ) continue;  
                //    		    
