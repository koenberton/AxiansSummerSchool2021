...
import org.tektonik.MachineLearning.ARFF.cmcTextToARFF;
...
public class cmcSMO {
    ...
    //------------------------------------------------------------
    public boolean testSMO(String OriginalFileName , String LongModelName , cmcSMODTOCore params)
    //------------------------------------------------------------
    {
        ...
        //
        cmcSMODTO smodto = new cmcSMODTO( OriginalFileName , DataTargetTuple[0].getNbrOfRows() , params.getC() , params.getTolerance() , params.getMaxCycles() , params.getKernelTrickTipe() );
        smodto.setBias(0);
...
do_error( "Ensure classname list matches sequence -1 and 1");
     ...
     try {
        ...
        case RBF  : {
        	...
        	if( dto.getKernelTrickTipe() == cmcMachineLearningEnums.KernelType.GAUSSIAN ) {
        		factor = 1 / ((double)2 * dto.getSigma() * dto.getSigma());
        	}
   ...
   //------------------------------------------------------------
    ...
    //------------------------------------------------------------
    {
    	if( dto.isInitializedOK() == false ) {
    		do_error("SMO DTO not initialized correctedly " + dto.getError() );
    		return false;
    	}
    	cmcMatrix dataMatrix = dto.getDataMatrix();
    	cmcMatrix resultMatrix = dto.getResultMatrix();
