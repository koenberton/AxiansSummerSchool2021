...
import org.tektonik.MachineLearning.Bayes.cmcBayesModel;
import org.tektonik.MachineLearning.DecisionTree.DecisionTreeDTO;
import org.tektonik.MachineLearning.Evaluation.cmcModelEvaluation;
import org.tektonik.MachineLearning.KNN.cmcKNearestNeighboursDTO;
import org.tektonik.MachineLearning.LogisticRegression.cmcLogisticRegressionDTO;
import org.tektonik.MachineLearning.SVM.cmcSMODTO;
...
public class cmcMachineLearningModelDAO {
    ...
    //------------------------------------------------------------
    {
    	...
    	for(int i=0;i<models.length ; i++)
    	{
        	...
        	if( (model.tipe == ARFF_TYPE.NOMINAL) || (model.tipe == ARFF_TYPE.CLASS) ) {
    ...
    //------------------------------------------------------------
    {
     	 ...
     	 try {
 	        ...
 	        reader = new BufferedReader(new InputStreamReader(new FileInputStream(inFile), xMSet.getCodePageString()));
 	          ...
 	          if( sLijn.indexOf("<Classes>") >= 0 ) {
 	        	 classlist = xMSet.xU.getSquaredItems(  xMSet.xU.extractXMLValue(sLijn,"Classes") );
 	        	 continue;
 	          }
 	          ...
 	          if( sLijn.indexOf("<ModelIndex>") >= 0 ) {
 				...
 				models[idx] = new cmcBayesModel( NumberOfBins , NumberOfClasses , "CATEGORYNAME-UNKNOWN" , ARFF_TYPE.UNKNOWN );
 	          ...
 	          if( sLijn.indexOf("<CategoryType>") >= 0 ) {
 	        	 ...
 	        	 if( sTipe.compareToIgnoreCase("NUMERIC") == 0 ) models[idx].tipe = ARFF_TYPE.NUMERIC;
 	        	 else
 	        	 if( sTipe.compareToIgnoreCase("NOMINAL") == 0 ) models[idx].tipe = ARFF_TYPE.NOMINAL;
 	        	 else
 	        	 if( sTipe.compareToIgnoreCase("CLASS") == 0 ) models[idx].tipe = ARFF_TYPE.CLASS;
 	        	 else {
 	          ...
 	          if( sLijn.indexOf("<NominalValues>") >= 0 ) {
 	        	 ...
 	        	 models[idx].NominalNameList = xMSet.xU.getSquaredItems( sNomi );
   	 ...
   	 //for(int z=0;z<models[idx].NominalNameList.length;z++) do_log( 1 , " -> " + models[idx].NominalNameList[z]);
 	          ...
 	          if( sLijn.indexOf("<BinSegmentBegins>") >= 0 ) {
 	        	 String[] lst = xMSet.xU.getSquaredItems( xMSet.xU.extractXMLValue(sLijn,"BinSegmentBegins") );
 	          ...
 	          if( sLijn.indexOf("<BAClassCountPerBin>") >= 0 ) {
 	        	 ...
 	        	 String[] lst = xMSet.xU.getSquaredItems( xMSet.xU.extractXMLValue(sLijn,"BAClassCountPerBin") );
 	        	 if( lst == null ) continue;
    ...
    //------------------------------------------------------------
    {
        ...
        for(int i=0;i<root.categories.length;i++)
        {
        	...
        	if( sub.getTipe() == ARFF_TYPE.CLASS ) {
    		...
    		if( bra == null ) continue;
 		    ...
 		    if( (bra.treetipe == cmcMachineLearningEnums.DecisionTreeType.ROOT) || (bra.treetipe == cmcMachineLearningEnums.DecisionTreeType.BRANCH) ) {
 		    	aps.println("<SplitOnCategory>" + bra.splitOnCategoryName + "</SplitOnCategory>");
  		 		...
  		 		if( bra.splitOnARFFTipe == ARFF_TYPE.NOMINAL ) {
    ...
    //------------------------------------------------------------
    {
        ...
        if( curnode.splitOnARFFTipe == ARFF_TYPE.NOMINAL ) sl += " Val=" + curnode.splitOnNominalName;
    ...
    //------------------------------------------------------------
    private ARFF_TYPE fetchARFFType(String stip)
    //------------------------------------------------------------
    {
     	  if( stip == null ) return null; 
      	  if ( stip.trim().compareToIgnoreCase("NUMERIC") == 0 ) return ARFF_TYPE.NUMERIC;
          if ( stip.trim().compareToIgnoreCase("NOMINAL") == 0 ) return ARFF_TYPE.NOMINAL;
          if ( stip.trim().compareToIgnoreCase("STRING") == 0 ) return ARFF_TYPE.STRING;
          if ( stip.trim().compareToIgnoreCase("CLASS") == 0 ) return ARFF_TYPE.CLASS;
          if ( stip.trim().compareToIgnoreCase("DATE") == 0 ) return ARFF_TYPE.DATE;
      	  return null;
    }
          ...
          BufferedReader reader=null;
      	  try {
  	        ...
  	        reader = new BufferedReader(new InputStreamReader(new FileInputStream(inFile), xMSet.getCodePageString()));
  	          ...
  	          if( sLijn.indexOf("<Classes>") >= 0 ) {
  	       	    ClassNameList = xMSet.xU.getSquaredItems(  xMSet.xU.extractXMLValue(sLijn,"Classes") );
  			  ...
  			  */
  	          ...
  	          if( sLijn.indexOf("<SplitOnType>") >= 0 ) {
  				...
  				if( (nnode<0) || (nnode > model.length)) { do_error("Node overrun"); return  null; }
  				if( stip.compareToIgnoreCase("NUMERIC") == 0 ) model[ nnode ].splitOnARFFTipe = ARFF_TYPE.NUMERIC;
  				else
  				if( stip.compareToIgnoreCase("NOMINAL") == 0 ) model[ nnode ].splitOnARFFTipe = ARFF_TYPE.NOMINAL;
  	  			else {
    ...
    //------------------------------------------------------------
    public boolean checkCategories( ARFFCategoryCoreDTO[] categories , int ncat)
    //------------------------------------------------------------
    {
    	...
    	for(int i=0;i<categories.length;i++)
    	{
    		...
    		if( (cat.getTipe() == ARFF_TYPE.CLASS) || (cat.getTipe() == ARFF_TYPE.NOMINAL) ) {
    		...
    		if( (cat.getTipe() == ARFF_TYPE.NUMERIC) || (cat.getTipe() == ARFF_TYPE.STRING) ) {
    			String[] list = cat.getNominalValueList();
    ...
    //------------------------------------------------------------
    public boolean dumpARFFCategoryCoreDTOList(ARFFCategoryCoreDTO[] flist , gpPrintStream aps)
    //------------------------------------------------------------
    {
    ...
    //------------------------------------------------------------
    public boolean parseCategory(  String sLijn , ARFFCategoryCoreDTO[] categories )
    //------------------------------------------------------------
    {
    	 ...
    	 //
       	 ...
       	 //
       	 if( sLijn.indexOf("<CategoryType>") >= 0 ) {
       	    ARFF_TYPE tipe = fetchARFFType( xMSet.xU.extractXMLValue(sLijn,"CategoryType") );
       	 ...
       	 if( sLijn.indexOf("<NominalValueList>") >= 0 ) {
				String[] slist = xMSet.xU.getSquaredItems(  xMSet.xU.extractXMLValue(sLijn,"NominalValueList") );
				if( slist.length < 1 ) {
     	 ...
     	 try {
 	        ...
 	        reader = new BufferedReader(new InputStreamReader(new FileInputStream(inFile), xMSet.getCodePageString()));
    	      ...
    	      if( sLijn.indexOf("<Weights>") >= 0 ) {
	  				String[] swlist = xMSet.xU.getSquaredItems(xMSet.xU.extractXMLValue(sLijn,"Weights"));
    ...
    //------------------------------------------------------------
    {
    	...
    	try {
    		...
    		if( dto.getKernelTrickTipe() == cmcMachineLearningEnums.KernelType.GAUSSIAN )
    		 aps.println("<GaussSigma>" + dto.getSigma() + "</GaussSigma>");
    		if( dto.getKernelTrickTipe() == cmcMachineLearningEnums.KernelType.RBF )
    		 aps.println("<RBFGamma>" + dto.getGamma() + "</RBFGAmma>");
    		aps.println("</SMOModelGeneral>");
    		//
         ...
         BufferedReader reader=null;
     	 try {
 	        ...
 	        reader = new BufferedReader(new InputStreamReader(new FileInputStream(inFile), xMSet.getCodePageString()));
    	      ...
    	      //
    	      if( sLijn.indexOf("<GaussSigma>") >= 0 ) {
	  				double dd = xMSet.xU.NaarDoubleNAN( xMSet.xU.extractXMLValue(sLijn,"GaussSigma"));
	  				if( Double.isNaN(dd) ) { do_error("Wrong sigma " + sLijn ); return null; }
	  				model.setSigma(dd);
	  				continue;	 
	  		  }
    	      //
    	      if( sLijn.indexOf("<RBFGamma>") >= 0 ) {
	  				double dd = xMSet.xU.NaarDoubleNAN( xMSet.xU.extractXMLValue(sLijn,"RBFGamma"));
	  				if( Double.isNaN(dd) ) { do_error("Wrong gamma " + sLijn ); return null; }
	  				model.setGamma(dd);
	  				continue;	 
	  		  }
    	      //
