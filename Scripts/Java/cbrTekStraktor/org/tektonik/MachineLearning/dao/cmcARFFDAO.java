...
import org.tektonik.MachineLearning.ARFF.cmcARFF.ARFFattribute;
...
public class cmcARFFDAO {
	...
	private enum GRANULARITY { UNIQUE , DUPLICATE , COLLISION }
	
	private static int MAX_NBR_OF_RECORDS = 7500;
	...
	private String TriStateValue = "?";
	private boolean GotTriStates=false;
	...
	//------------------------------------------------------------
	{
		 ...
		 if( sl.startsWith("%") ) {
      	   ...
      	   if( sl.startsWith("%%") ) return true;  // collisions
      	   if( relationComment==null ) {
      		   String ss = xMSet.xU.Remplaceer(sLijn,"%","").trim();
      		   if( ss.length() > 0 ) relationComment = ss; 
      	   }
      	   if( ALLOW_DUPLICATES==false) return true;
      	   int n = xMSet.xU.TelDelims( sl , ',' );
      	   if( ((n != DAOnattribs) && (indata)) ||(!indata) ) { // genuine comment
    ...
    //------------------------------------------------------------
    {
    	...
    	try {
	        ...
	        reader = new BufferedReader(new InputStreamReader(new FileInputStream(inFile), xMSet.getCodePageString()));
	           ...
	           //
	           if( processComment(sLijn,indata) ) continue;
	           //
	           if( indata ) {
	        	   DAOndatarows++;
	        	   //
	        	   if( GotTriStates ) continue;
	        	   StringTokenizer st = new StringTokenizer(sLijn,",");
	        	   while(st.hasMoreTokens()){
	        		 String sElem = st.nextToken().trim();
	        		 if( sElem.compareToIgnoreCase(TriStateValue)==0 ) { GotTriStates=true; break; }
	        	   }
	           }
	       	}
    	}
    ...
    //------------------------------------------------------------
    {
	           ...
	           if( sl.startsWith("@ATTRIBUTE ") || (sl.startsWith("@ATTRIBUTE\t")) ) {
	        	 try {   
	        	   ...
	        	   while(st.hasMoreTokens()){
	        		 ...
	        		 if( idx == 2 ) {
	        		   if( sElem.compareToIgnoreCase("NUMERIC") == 0 )  xa.getAttributeList()[attribCount].setTipe(ARFF_TYPE.NUMERIC);
	        		   else
	        		   if( sElem.compareToIgnoreCase("REAL") == 0 )  xa.getAttributeList()[attribCount].setTipe(ARFF_TYPE.NUMERIC);
	        		   else
	        		   if( sElem.compareToIgnoreCase("INTEGER") == 0 )  xa.getAttributeList()[attribCount].setTipe(ARFF_TYPE.NUMERIC);
		        	   else
	        		   if( sElem.compareToIgnoreCase("STRING") == 0 )  xa.getAttributeList()[attribCount].setTipe(ARFF_TYPE.STRING);
	        		   else
	        		   if( (sElem.startsWith("{")) && (sl.indexOf("}")>0) ) {
	        			  int zdx = sLijn.indexOf('}');
	        			  sElem = sLijn.substring( sLijn.indexOf('{') , (zdx+1) );
	        			  //do_error( "[" + sElem + "] [" + sLijn + "] " +(zdx+1) );
	        			  xa.getAttributeList()[attribCount].setTipe( ARFF_TYPE.CLASS );
	        			  ...
	        			  int pdx=-1;
	   	        	      ...
	   	        	      if( (TriStateValue != null) && (GotTriStates) ) {  // add ? if needed
	   	        	    	  boolean found = false;
	   	        	    	  for(int i=0;i<cats.length;i++)
	   	        	    	  {
	   	        	    		  if( cats[i].compareToIgnoreCase(TriStateValue) == 0) { found=true; break; }
	   	        	    	  }
	   	        	    	  if( !found ) {
	   	        	    		  String[] ExtendedCats = new String[ cats.length + 1];
	   	        	    		  for(int i=0;i<cats.length;i++) ExtendedCats[i] = cats[i];
	   	        	    		  ExtendedCats[cats.length] = TriStateValue;
	   	        	    		  cats = null;
	   	        	    		  cats = ExtendedCats;
	   	        	    	  }
	   	        	      }
	           ...
	           if( indata ) {
	         	 try {
	        	   ...
	        	   while(st.hasMoreTokens()){
	        	       ...
	        	       // store the value
	        	       if( xa.getAttributeList()[ idx ].getTipe() == ARFF_TYPE.STRING ) {
	        		     xa.getAttributeList()[ idx ].setSValue( rowCount , sElem.trim());
	        		   }
	        	       else
	        	       if( xa.getAttributeList()[ idx ].getTipe() == ARFF_TYPE.NUMERIC ) {
	        		     xa.getAttributeList()[ idx ].setValue( rowCount , xMSet.xU.NaarDouble( sElem ));
	        		     if( xa.getAttributeList()[ idx ].getValue( rowCount ) == -1 ) {
	        		       //if( sElem.trim().startsWith("?") == true ) {
	        		       //	   xa.getAttributeList()[ idx ].setValue( rowCount , -99999 );	 
	        		       //}
	        		       //else
	        		       // issue indien value == -1
	        		       if( sElem.trim().startsWith("-1") == false ) {
	        			     do_log(1,"Something went wrong [" + sElem + "] on [" + sLijn + "] - could not transform to double" );
	        		       }
	        		     }
	        	       }
	        	       // find and store the index of the class/category
	        	       else if( xa.getAttributeList()[ idx].getTipe() == ARFF_TYPE.CLASS ) {
	        	    	     int pdx=-1;
    	...
    	if( xa != null ) {
    		 ...
    		 if( xcat.getTipe() != ARFF_TYPE.CLASS ) {
    		 ...
    		 for(int i=0 ; i<(xa.getAttributeList().length-1) ; i++ )
    		 {
    		    ...
    		    if( xcat.getTipe() == ARFF_TYPE.CLASS ) { xcat.setTipe( ARFF_TYPE.NOMINAL ); }
    		 }
    	}
    	//
    ...
    //------------------------------------------------------------
    {
    	...
    	for(int i=0; i < xa.getAttributeList().length ; i++ )
    	{
    		 ...
    		 if( atr.getTipe() == ARFF_TYPE.CLASS ) {
    			String sl="";
    			for(int j=0;j<atr.getClasses().length;j++) sl += "[" + atr.getClasses()[j] + "]";
    			do_log( 1 , "-->" + sl);
    		 }
    		 if( atr.getTipe() != ARFF_TYPE.STRING ) {
    		  String svl="";
    ...
    //------------------------------------------------------------
    {
    	...
    	{
    	  String sl = "";
    	  if( (xa.getAttributeList()[i].getTipe() == ARFF_TYPE.CLASS) || (xa.getAttributeList()[i].getTipe() == ARFF_TYPE.NOMINAL) ) {
    	...
    	for(int i=0;i<xa.getAttributeList()[0].getValues().length;i++)  // attribute 0 has same number of rows as any other
    	{
    		...
    		{
    		  if( j != 0 ) sl += ",";
    		  if( xa.getAttributeList()[j].getTipe() == ARFF_TYPE.STRING ) sl += xa.getAttributeList()[j].getSValue(i);
    		  else
    		  if( xa.getAttributeList()[j].getTipe() == ARFF_TYPE.NUMERIC ) sl += xa.getAttributeList()[j].getValue(i);	
    		  else
    		  if( xa.getAttributeList()[j].getTipe() == ARFF_TYPE.NOMINAL ) sl += xa.getAttributeList()[j].getSValue(i);
    		  else
        	  if( xa.getAttributeList()[j].getTipe() == ARFF_TYPE.CLASS ) sl += xa.getAttributeList()[j].getSValue(i);
        	  else {
  ...
  //do_log( 1 , "" + xa.getAttributeList()[i].getTipe()  + " " + xa.getAttributeList()[i].getAttrName());
           if( xa.getAttributeList()[i].getTipe() == ARFF_TYPE.STRING )  continue;
           if( xa.getAttributeList()[i].getTipe() == ARFF_TYPE.CLASS )  continue;
           cnt++;
           if( idx < 0 ) idx = i;     
    	}
    	if( cnt == 0 ) {
    	...
    	ARFFattribute pivot = xa.getAttributeList()[idx];
    	...
    	for( int row=0;row<ndatarows; row++)
    	{
    		for(int i=0;i<ndatarows;i++)
    		{
    			...
    			{
    			  ARFFattribute x = xa.getAttributeList()[j];
    			  if( x.getTipe() == ARFF_TYPE.STRING )  continue;
    			  if( x.getTipe() == ARFF_TYPE.CLASS )  continue;
    	          if( x.getTipe() == ARFF_TYPE.NUMERIC )  {
    	          if( xa.getAttributeList()[j].getValue(row) != xa.getAttributeList()[j].getValue(i) ) duper[i] = false;
    	          } 
    	          if( x.getTipe() == ARFF_TYPE.NOMINAL )  {
        	          if( xa.getAttributeList()[j].getValue(row) != xa.getAttributeList()[j].getValue(i) ) duper[i] = false;
        	      } 
    			}  
    		}
    	...
    	if(  xa.getAttributeList()[classidx].getTipe() != ARFF_TYPE.CLASS ) {
