...
import org.tektonik.tools.generalStatPurpose.gpStatAdditionalFunctions;
...
public class ARFFVisualizer {
    ...
    //------------------------------------------------------------
    {
        ...
        for(int i=0 ; i<nattribs ; i++)
        {
        	...
        	if( attrib.getTipe() == ARFF_TYPE.STRING ) continue;
        	if( attrib.getTipe() == ARFF_TYPE.CLASS ) {
        ...
        if( gdto == null ) return true;  // sometimes this function is called without GDTO being populated
    ...
    //------------------------------------------------------------
    {
        ...
        for(int i=0 ; i<gdto.getCategories().length ; i++)
        {
         	...
         	if( cat.getTipe() == ARFF_TYPE.STRING ) continue;
         	nvalids++;
        }
    ...
    //------------------------------------------------------------
    {
        ...
        drwtxt.setBackGroundColor( Color.WHITE );
        ...
        drw.pasteSnippet( canvas , (MARGIN/2) , (MARGIN/2) , canvas_width , nbrpix , diarout.getTileWidth() );
        //
        int cnt=0;
        for(int i=0 ; i<gdto.getCategories().length ; i++)
        {
            ...
            drw.pasteSnippet( canvas , cx , cy , canvas_width , pix , diarout.getTileWidth() );
            cnt++;
            pix = null;
        }
    ...
    //------------------------------------------------------------
    {
        ...
        for(int i=0 ; i<gdto.getCategories().length ; i++)
        {
        	...
        	if( cat1.getTipe() == ARFF_TYPE.STRING ) continue;
        	y++;
        	int x=-1;
        	for(int j=0;j<gdto.getCategories().length;j++)
        	{
            	...
            	if( cat2.getTipe() == ARFF_TYPE.STRING ) continue;
            	x++;
            	int[] pix = null;
                ...
                drw.pasteSnippet( canvas , cx , cy , canvas_width , pix , diarout.getRectangleSide() );
                pix=null;
        	}
        }
        // ARGB indien transparency
    ...
    //------------------------------------------------------------
    {
    	...
    	{
    	   cat1 = gdto.getCategories()[i];
    	   if( cat1.getTipe() == ARFF_TYPE.STRING ) { correlationMatrix[ i ][ i ] = 1; continue; }
    	   if( cat1.getTipe() == ARFF_TYPE.CLASS ) { correlationMatrix[ i ][ i ] = 1; continue; }
    	   if( cat1.getTipe() == ARFF_TYPE.NOMINAL ) { correlationMatrix[ i ][ i ] = 1; continue; }
    	   for(int j=0; j < gdto.getCategories().length;j++)
    	   {
    		   cat2 = gdto.getCategories()[j];
    		   if( cat2.getTipe() == ARFF_TYPE.STRING ) continue;
    		   if( cat2.getTipe() == ARFF_TYPE.CLASS ) continue;
    		   if( cat2.getTipe() == ARFF_TYPE.NOMINAL ) continue;
    ...
    //------------------------------------------------------------
    {
        	 ...
        	 if( cat1 == null ) return null;
       	     if( cat1.getTipe() == org.tektonik.MachineLearning.ARFF.ARFFEnums.ARFF_TYPE.STRING ) return null;
       	     if( cat2 == null ) return null;
    	     if( cat2.getTipe() == org.tektonik.MachineLearning.ARFF.ARFFEnums.ARFF_TYPE.STRING ) return null;
    	     if( resorted == false ) {
    	    	 if ( performResort() == false) return null;
    	     }
