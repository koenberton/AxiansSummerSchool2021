...
public class cmcARFFRoutines {
    ...
    //------------------------------------------------------------
    {
    	...
    	for(int i=0;i<xa.getAttributeList().length;i++)
    	{
    		...
    		if( (attr.getTipe() != ARFF_TYPE.NUMERIC) && (attr.getTipe() != ARFF_TYPE.NOMINAL) ) continue;
    	    if( ndatarows == -1 ) {
    ...
    //------------------------------------------------------------
    {
        ...
        if( RANDOMIZE ) {
          ...
          int threshold = (int)(((double)ndatarows * (double)cmcMachineLearningConstants.TEST_SET_PERCENTAGE ) / (double)100);
    ...
    //------------------------------------------------------------
    {
    	...
    	*/
    	String FileNameTrainingSet = xMSet.getARFFFileNameTrainingSet(OriginalFileName);
    	if( FileNameTrainingSet == null ) return null;
    	String FileNameTestSet = xMSet.getARFFFileNameTestSet(OriginalFileName);
    	if( FileNameTestSet == null ) return null;
    	...
    	if( splitFileRequested ) {
    		...
    		FileNameTrainingSet = OriginalFileName;
    	...
    	if( last.getTipe() != ARFF_TYPE.CLASS ) {
