...
import org.tektonik.MachineLearning.cmcModelMaker;
...
import org.tektonik.MachineLearning.dao.cmcMachineLearningModelDAO;
import org.tektonik.tekStraktor.featureExtraction.PredictDetail;
...
public class cmcModelmakerDecisionTree implements cmcModelMaker {
	...
	public class DecisionTreeCategoryBranch
	{
	   ...
	   public ARFF_TYPE tipe= ARFF_TYPE.UNKNOWN;
	   public String[] NominalNameList;
	   double TotalEntropy;
	   DecisionTreeMetrics[] detail;
	   
	   DecisionTreeCategoryBranch(String iname , ARFF_TYPE itipe , String[] inomlist)
	   {
		   CategoryName = iname;
		   tipe = itipe;
	   ...
	   public boolean calculateEntropyGains(String[] ClassNameList)
	   {
		   ...
		   if( this.tipe == ARFF_TYPE.STRING) return true;
		   if( this.tipe == ARFF_TYPE.CLASS) return true;
	...
	public class DecisionTreeBranch
	{
		...
		public DecisionTreeBranch( int ilevel , long lparent , ARFFCategoryCoreDTO[] icategories , cmcMachineLearningEnums.BranchOrientation iorient)
		{
			...
			dto.splitOnARFFTipe = ARFF_TYPE.UNKNOWN;
		...
		private boolean findBestGain()
		{
			...
			for(int i=0;i<subbranches.length;i++)
			{
				...
				if( (sub.tipe == ARFF_TYPE.STRING) || (sub.tipe == ARFF_TYPE.CLASS) ) continue;
			...
			if( this.dto.splitOnARFFTipe  == ARFF_TYPE.NOMINAL ) {
		...
		public boolean calculateAllEntropyGains()
		{
			...
			{
				DecisionTreeCategoryBranch sub = this.subbranches[i];
				if( sub.tipe == ARFF_TYPE.STRING) continue;
				if( sub.tipe == ARFF_TYPE.CLASS ) {
...
do_log( 1 , "Max E-Gain [Node="+ this.dto.UID + "] ["  +
    ...
    //------------------------------------------------------------
    {
    	...
    	for(int i=0;i<root.subbranches.length;i++)
    	{
    		...
    		if( sub.tipe == ARFF_TYPE.STRING ) { // just use the lineno as the value -- probably not necessary
  ...
  //------------------------------------------------------------
    ...
    //------------------------------------------------------------
    {
    	...
    	for(int i=0;i<nattribs;i++)
    	{
    	   ...
    	   if( attrib.getTipe() == ARFF_TYPE.CLASS ) continue;
        ...
        if( xa.getAttributeList()[ xa.getAttributeList().length - 1].getTipe() != ARFF_TYPE.CLASS ) {
  	    ...
  	    for(int row=0; row < ndatarows ; row++)
    	{
    	   ...
    	   for(int col=0;col<nattribs;col++)
    	   {
    		  ...
    		  if( attrib == null ) continue;
    		  if( (attrib.getTipe() == ARFF_TYPE.STRING) && (FileName==null) ) {
  				FileName = xa.getAttributeList()[col].getSValue( row );
  			  }
    		  if( attrib.getTipe() == ARFF_TYPE.STRING ) continue;
       	      if( attrib.getTipe() == ARFF_TYPE.CLASS ) { actual = attrib.getSValue(row); continue; }
