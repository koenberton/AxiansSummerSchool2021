package org.tektonik.tools.generalImagePurpose.analysis;
 ...
 * </ul>
 */
public class gpFetchByteImageData {
...
*/
    ...
    private static void print(String sourceName, gpFetchByteImageData ii, boolean verbose) {
        if (verbose) {
    ...
    private static void printCompact(String sourceName, gpFetchByteImageData imageInfo) {
        MyInfo = imageInfo.getFormatName() + ";" +
    ...
    /*
    private static void printVerbose(String sourceName, cmcProcImageInfo ii) {
        ...
        printLine(1, "Physical width (centimeter): ", ii.getPhysicalWidthInch() * 2.54f, 1.0f);
        printLine(1, "Physical height (centimeter): ", ii.getPhysicalHeightInch() * 2.54f, 1.0f);
    ...
    */
    private static void printVerbose(String sourceName, gpFetchByteImageData ii) {
        //printLine(0, null, sourceName);
    	MyInfo = "";
        MyInfo +=  "[File format: "+ ii.getFormatName();
        MyInfo +=  "] [MIME type: " + ii.getMimeType();
        MyInfo +=  "] [Width (pixels): " + ii.getWidth();
        MyInfo +=  "] [Height (pixels): " + ii.getHeight();
        MyInfo +=  "] [Bits per pixel: " + ii.getBitsPerPixel();
        MyInfo +=  "] [Progressive: " + Boolean.toString(ii.isProgressive());
        MyInfo +=  "] [Number of images: " + ii.getNumberOfImages();
        MyInfo +=  "] [Physical width (dpi): " +ii.getPhysicalWidthDpi();
        MyInfo +=  "] [Physical height (dpi): " + ii.getPhysicalHeightDpi();
        MyInfo +=  "] [Physical width (inches): " + ii.getPhysicalWidthInch();
        MyInfo +=  "] [Physical height (inches): " + ii.getPhysicalHeightInch();
        MyInfo +=  "] [Physical width (centimeter): " + ii.getPhysicalWidthInch() * 2.54f;
        MyInfo +=  "] [Physical height (centimeter): " + ii.getPhysicalHeightInch() * 2.54f;
        int numComments = ii.getNumberOfComments();
        MyInfo += "][Number of textual comments: " + numComments;
        if (numComments > 0) {
            for (int i = 0; i < numComments; i++) {
            	MyInfo += " ["+ ii.getComment(i) + "]";
            }
        }
        MyInfo += "]";
    }
    ...
    private static void run(String sourceName, InputStream in, gpFetchByteImageData imageInfo, boolean verbose) {
    ...
    public void cmcProcGetImageInfo(String FNaam ) 
    {
        ...
        //ImageInfo imageInfo = new ImageInfo();
        this.setDetermineImageNumber(true);
        boolean verbose = true;
        
        InputStream in = null;
        try {
            if (FNaam.startsWith("http://")) {
                in = new URL(FNaam).openConnection().getInputStream();
            } else {
                in = new FileInputStream(FNaam);
            }
            run(FNaam, in, this, verbose);
            in.close();
        } catch (Exception e) {
            System.out.println(e);
            try {
