...
public class CannyEdgeDetector {
	...
	//-------------------------------------
	// CBRTEKSTRAKTOR changes
	//--------------------------------------
	public int[] cmcProcess(int[] imgpx , int iw) 
	{
		try {
		width = iw;
		height = imgpx.length / width;
		picsize = width * height;
		initArrays();
        //System.err.println("" + width + " " + height + " " + data.length );
		//readLuminance();
		cmcReadLuminance(imgpx);
		...
		thresholdEdges();
		//writeEdges(data);
	    // move data back to picture format	
		for(int i=0;i<data.length;i++)
		{
			int p = data[i] & 0x000000ff;
	        data[i] = (255 << 24) | (p << 16) | (p << 8) | p; 	
	    }
		return data;
	...
	private void cmcReadLuminance(int[] imgpx) {
		int[] pixels = new int[ imgpx.length ];
		for(int i=0;i<pixels.length;i++) pixels[i] = imgpx[i];
