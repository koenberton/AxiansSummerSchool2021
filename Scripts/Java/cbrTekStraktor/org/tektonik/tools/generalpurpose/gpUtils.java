...
public class gpUtils {
	...
	//---------------------------------------------------------------------------------
	public String GetShortFileName(String LongFileName)
	//---------------------------------------------------------------------------------
	{         
		try {
		int idx = LongFileName.lastIndexOf( ctSlash );
		if( idx < 0 ) return LongFileName;
		String shortName = LongFileName.substring( idx + 1 );
		return shortName;
		}
	...
	//---------------------------------------------------------------------------------
	public double NaarDoubleNAN(String sIn)
	//---------------------------------------------------------------------------------
	{
	...
	{  int teller=0;
		...
		//logit( 0 , "removed [" + sIn + "]");
			...
			//---------------------------------------------------------------------------------
			public String getFileNameWithoutSuffix(String sAbs)
			//---------------------------------------------------------------------------------
			{
				try {
					int idx = sAbs.lastIndexOf( "." );
					String FName = sAbs.substring( 0 , idx );
					return FName;
				}
				catch (Exception e ) { return null;	}
			}
			...
			//---------------------------------------------------------------------------------
            public String getField(String sin, int idx , String Delims)
            //---------------------------------------------------------------------------------
            {   // Delims = " ,|" 
            	try  {
            	int i=0;
            	StringTokenizer tokenizer = new StringTokenizer(sin, Delims);
        	...
        	//---------------------------------------------------------------------------------
            public String getNextImageInDir( String sDir , String FName , boolean right)
        	//---------------------------------------------------------------------------------
            {
            	try {
            	if( (sDir == null) || (FName == null)  ) return null;
            	if( !IsDir(sDir)  ) return null;
            	String sTarget = sDir + ctSlash + FName;
            	if( !IsBestand( sTarget ) ) { System.err.println( "no file - " + sTarget ); return null; }
            	ArrayList<String> all = GetFilesInDir( sDir , null );
            	if( all == null ) return null;
            	if( all.size() == 0 ) return null;
            	ArrayList<String> list = new ArrayList<String>();
            	for(int i=0;i<all.size();i++)
            	{
                    //System.err.println( all.get(i) + " " + isGrafisch( all.get(i)) );
            		if( isGrafisch( all.get(i) )) list.add( all.get(i) );
            	}
            	if( list.size() <= 1 ) return null;  // 1 must be the same
            	int idx=-1;
            	for(int i=0;i<list.size();i++)
            	{
            		if( list.get(i).compareToIgnoreCase(FName) == 0 ) { idx=i; break; }
            	}
            	if( idx < 0 ) return null;
            	idx = right ? idx + 1 : idx -1;
            	if( idx >= list.size() ) idx = 1;
            	if( idx < 0 ) idx = list.size() -1;
            	return sDir + ctSlash + list.get( idx );
            	}
            	catch(Exception e) { return null; }
            }
            
            //---------------------------------------------------------------------------------
            public String[] getSquaredItems( String sin )
            //---------------------------------------------------------------------------------
            {
            	return strUtil.getSquaredItems(sin);
            }
}