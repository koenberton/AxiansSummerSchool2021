package org.tektonik.tools.generalGUIPurpose.windows;
...
import org.tektonik.tools.generalGUIPurpose.widgets.ClosableTabbedPane;
import org.tektonik.tools.generalGUIPurpose.widgets.CloseIcon;
...
public class cmcFileViewer {
	...
	private boolean FontZoomer=false;
	...
	//-----------------------------------------------------------------------
	{
		    ...
		    JPanel p = new JPanel();
		       try {
	            ...
	            zz.editor.setFont(new Font( FontName , Font.PLAIN, FontSize));
	...
	//-----------------------------------------------------------------------
	{
		try {
			 ...
			 FontZoomer=false;
		     boolean ib =updateURL(  zz.URL , oldzoom , zz.zoom );
		     if( ib == false ) {
		    	 if( FontZoomer == false ) return;
		    	 FontSize += step;
	...
	//-----------------------------------------------------------------------
	{
		...
		if( header.indexOf("ZOOMABLE") < 0 ) { 
			//popMessage("File is not zoomable. Only font will be modified\n" + LongFileName ); 
			FontZoomer=true;
			return false; 
		}
